...
|---|---|---|
| CLI surface | มี `serve`, `connect`, `cp`, `devices`, `version` ใน `src/main.rs`; local fixture และ remote `serve`/`connect --target`/`cp --target` เรียก signaling, WebRTC, authenticated runtime และ bounded stream service จริง | Local plus deterministic remote relay implemented; external provider interoperability pending |
...
| Security threat model | Issue #46 เพิ่ม ADR `docs/decisions/issue-46-threat-model.md` ครอบคลุม identity/pairing, signaling, WebRTC/session, file, shell, proxy, bounded response handling, dependencies, logging และ platform boundary โดยทุก finding มี severity, evidence, owner และ disposition; Issue #65 เพิ่ม incremental HTTP response reading และ chunked-response regression test; Issue #67 แก้ remote-first session shutdown race ด้วยการ tolerate เฉพาะ `send SWSP frame: data channel closed` และเพิ่ม regression unit test; Issue #73 แก้ review findings เรื่อง early PIN validation, fingerprint binding และ nonce replay; `docs/api.md` เพิ่ม capability security boundaries | Local code/tests มี hardening evidence รวม bounded response body และ idempotent session shutdown; dependency advisory scan, Windows/Android runner evidence และ original-Go/provider interoperability ยัง pending |
...
| Security boundary | Issue #46 กำหนดว่า authenticated `Ready` state และ capability-specific policy ต้องผ่านก่อนเปิด file/shell/proxy; Issue #65 กำหนดให้ HTTP response อ่านด้วย incremental chunks และไม่ append เกิน `max_response_bytes`; Issue #67 กำหนดให้ remote-first session shutdown สำเร็จได้เมื่อส่ง FIN ไม่ได้เพราะ data channel ปิดแล้ว โดย error อื่นยังต้องถูกส่งต่อ; application controls ไม่แทน OS sandbox, ACL หรือ firewall และ `wss` ยังถูกปฏิเสธจนกว่าจะมี tested TLS connector | Threat model versioned; bounded response และ bounded/idempotent local shutdown documented; production release gate ยังเปิด |
...
| 8 | เพิ่ม shell/HTTP/TCP/WebSocket, mDNS และ QR | **ผ่านบางส่วนใน Issues #40, #42 และ #71:** shell/file remote dispatcher และ concrete proxy handlers มี local/deterministic relay coverage; ยังเหลือ TCP/WebSocket dispatcher, TLS-enabled interoperability, mDNS/QR และ platform notes |
