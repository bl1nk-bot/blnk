...
**สถานะเอกสารฉบับนี้:** foundation runnable, identity/pairing primitives, SWSP raw frame codec, typed signaling/session/stream boundaries, deterministic protobuf generation boundary, local WebSocket signaling transport, WebRTC peer lifecycle สำหรับ local two-peer harness, authenticated session runtime, sandboxed file-transfer MVP, shell stream MVP, proxy policy/concrete handler boundaries, CLI local/remote orchestration, Issue #44 compatibility baseline, Issue #46 versioned threat model, Issue #47 loopback browser control surface และ Issue #67 Windows-safe session shutdown hardening ถูก implement ใน dependency branches แล้ว; Issue #73 เพิ่ม early remote PIN validation และ single-use encrypted-request challenge ใน PR #72 review path; reproducible release gate/artifacts ของ Issue #48, original-Go/provider interoperability, cross-platform release evidence, dependency advisory scan และ production readiness ยังไม่เสร็จ

## Executive Summary

blnk Rust **มี runnable local MVP ตามสถาปัตยกรรมแล้ว** และ Issue #7 เพิ่ม identity/pairing primitives, Issue #9 เพิ่ม SWSP raw frame codec, Issue #11 เพิ่ม typed signaling/session/stream boundaries, Issue #13 เพิ่ม deterministic protobuf generation boundary, Issue #33 เพิ่ม JSON-over-WebSocket transport กับ local loopback fixture ที่ทดสอบได้, Issue #35 เพิ่ม public-only signaling endpoint preflight พร้อม explicit local-fixture opt-in, Issue #37 เพิ่ม WebRTC offer/answer, non-trickle ICE, data-channel lifecycle และ deterministic local two-peer harness, Issue #38 เพิ่ม authenticated session runtime, Issue #39 เพิ่ม sandboxed file-transfer MVP, Issue #40 เพิ่ม shell stream MVP, Issue #41 เพิ่ม proxy security policy และ Issue #42 เพิ่ม concrete TCP/WebSocket/HTTP handler services บน policy boundary; Issue #43 เพิ่ม CLI local fixture orchestration และ metadata-only device registry, Issue #44 เพิ่ม versioned signaling/SWSP/control fixtures กับ authenticated local transcript, Issue #46 เพิ่ม threat model/audit checklist ที่ระบุ severity, evidence, owner, disposition และ residual risk, Issue #67 แก้ remote-first session shutdown race, Issue #73 เพิ่ม early PIN validation, SDP fingerprint binding และ single-use request nonce สำหรับ remote encrypted request, Issue #71 เพิ่ม remote signaling/WebRTC negotiation, authenticated session และ shell/file dispatcher ผ่าน deterministic relay fixture และ Issue #47 เพิ่ม loopback HTTP/WebSocket session-only browser control flow พร้อม exact Origin/CORS, bearer bootstrap, HttpOnly/SameSite cookie, CSRF และ bounded limits; ระบบยังไม่พร้อมอ้าง browser WebRTC/original-Go comparison, TLS/provider coverage, dependency advisory scan, release artifacts หรือ cross-platform release evidence [1] [2]
...
|---|---|---|
| CLI surface | มี `serve`, `connect`, `cp`, `devices`, `web`, `version` ใน `src/main.rs`; local fixture และ remote `serve`/`connect --target`/`cp --target` เรียก signaling, WebRTC, authenticated runtime และ bounded stream service จริง; `web` เปิด loopback browser control session/status surface | Local plus deterministic remote relay and loopback web control implemented; external/browser interoperability pending |
...
| Signaling boundary | Issue #11 มี typed register/request/offer/answer/candidate/pairing/error messages และ protocol-version validation; Issue #13 เพิ่ม generated protobuf bindings ใต้ namespace แยก; Issue #33 เพิ่ม JSON-over-WebSocket codec/client, message-size guard, close/error propagation, reconnect policy และ local loopback fixture tests; Issue #35 เพิ่ม default public-only endpoint preflight, DNS/IP special-use rejection และ explicit local-fixture opt-in; Issue #37 ใช้ SDP offer/answer แบบ non-trickle ใน process เป็น local harness boundary; Issue #44 เพิ่ม raw/versioned register fixture และ typed round-trip test; Issue #71 เชื่อม register/request/offer/answer กับ peer/session orchestration และ encrypted request metadata; Issue #73 ผูก request nonce กับ offer challenge, ตรวจ fingerprint/PIN ก่อน remote peer/session path และ consume challenge แบบ single-use | Local transport, deterministic remote relay and endpoint boundary implemented; original-Go/provider interoperability pending |
| Security threat model | Issue #46 เพิ่ม ADR `docs/decisions/issue-46-threat-model.md` ครอบคลุม identity/pairing, signaling, WebRTC/session, file, shell, proxy, bounded response handling, dependencies, logging และ platform boundary โดยทุก finding มี severity, evidence, owner และ disposition; Issue #47 เพิ่ม ADR `docs/decisions/issue-47-browser-control-surface.md` และ loopback browser boundary; Issue #65 เพิ่ม incremental HTTP response reading และ chunked-response regression test; Issue #67 แก้ remote-first session shutdown race ด้วยการ tolerate เฉพาะ `send SWSP frame: data channel closed` และเพิ่ม regression unit test; Issue #73 แก้ review findings เรื่อง early PIN validation, fingerprint binding และ nonce replay; `docs/api.md` เพิ่ม capability security boundaries | Local code/tests มี hardening evidence รวม browser origin/auth/CSRF/session/body/frame/rate limits, bounded response body และ idempotent session shutdown; dependency advisory scan, Windows/Android runner evidence และ browser/original-Go/provider interoperability ยัง pending |
| Peer lifecycle | Issue #37 มี `PeerHandle` สำหรับสร้าง peer, offer/answer, non-trickle ICE gathering, connected/failed/closed state, data-channel open/error/close, SWSP send/receive และ idempotent teardown; `TwoPeerHarness` พิสูจน์ local loopback path โดยไม่ใช้ external network; Issue #38 ใช้ `PeerHandle` เป็น control/data boundary ของ authenticated session runtime และ bounded graceful close; Issue #67 ทำให้ `SessionRuntime::close` ยอมรับ remote-first data-channel close แบบจำเพาะโดยไม่กลบ error อื่น; Issue #71 ใช้ API เดิมผ่าน signaling relay flow จริง | Local peer/session lifecycle and deterministic relay orchestration implemented; Windows runner, external ICE/provider interoperability pending |
| Session/auth | Issue #11 มี explicit connecting/authenticating/ready/closed state machine, PIN retry/delay policy, typed control messages และ cleanup on terminal failure; Issue #38 เพิ่ม `SessionRuntime` เชื่อม protobuf control messages กับ `PeerHandle`; Issue #39 ใช้ runtime boundary เดิมสำหรับ file request/data/FIN; Issue #43 ใช้ handshake จริงใน CLI local fixture; Issue #67 เพิ่ม safe remote-first shutdown หลัง authenticated handshake; Issue #71 ใช้ handshake เดิมหลัง remote SDP negotiation และ dispatch stream แบบ single-reader | Local runtime/file/CLI and deterministic remote relay integration implemented; Windows runner and original-client interoperability pending |
| Stream registry | Issue #11 มี non-zero stream ID allocation, lifecycle validation, counters และ cleanup; Issue #38 ผูก `open_stream`/`close_stream`/disconnect กับ runtime snapshot; Issue #39 เพิ่ม file streams; Issue #40 เพิ่ม shell streams; Issue #43 ใช้ทั้งสองผ่าน local CLI transcript; Issue #71 เพิ่ม single-reader remote shell/file dispatch ด้วย service เดิม | Registry/runtime/file/shell/CLI and deterministic remote dispatch implemented; external interoperability pending |
...
| Build | Linux validation gate ใช้ `cargo fmt --all -- --check`, `cargo check --all-targets`, `cargo test --all` และ `cargo clippy --all --all-targets -- -D warnings`; branch #47 รันครบหลังเพิ่ม web/WS feature | Passing on Linux branch #47; 119 library/integration tests ผ่าน, Windows/Android evidence ยังต้องอ่านจาก CI runner |
...
| Tests | มี unit tests สำหรับ config, identity, pairing, SWSP, signaling validation, session/auth lifecycle และ stream registry; Issues #33/#37/#38 เพิ่ม local transport, WebRTC, authenticated runtime และ failure-path coverage; Issue #39 เพิ่ม sandboxed file-transfer; Issue #40 เพิ่ม shell lifecycle/cancellation/limits; Issue #43 เพิ่ม CLI arg parsing, direct-argv safety, local authenticated shell/file transcripts และ metadata registry path; Issue #44 เพิ่ม 4 compatibility baseline tests และ versioned raw fixtures; Issue #47 เพิ่ม CLI parser, HTTP auth/origin/CORS/CSRF/session/rate/body tests และ WebSocket hello/status/frame tests; Issue #65 เพิ่ม chunked HTTP response limit regression test; Issue #67 เพิ่ม predicate test สำหรับ shutdown error และใช้ compatibility baseline remote-first close scenario เป็น regression target; Issue #71 เพิ่ม relay register/offer/answer/session, timeout/client-id/error และ remote shell/file E2E tests | Foundation plus local, loopback web and deterministic remote-relay coverage รวม wrong-PIN, wrong-nonce, wrong-fingerprint และ single-use challenge regression tests; browser/external interoperability pending |
| Release | Issue #48 ยังต้องเพิ่ม reproducible release gate, provenance/checksum, version/changelog, packaging และ release smoke evidence | Not started; #47 browser surface does not change this status |
...
| Security boundary | Issue #46 กำหนดว่า authenticated `Ready` state และ capability-specific policy ต้องผ่านก่อนเปิด file/shell/proxy; Issue #47 เพิ่ม loopback-only browser control ที่ใช้ exact Origin/CORS, bearer bootstrap, HttpOnly/SameSite cookie, CSRF, constant-time token comparison และ bounded session/body/frame/rate/TTL limits; Issue #65 กำหนดให้ HTTP response อ่านด้วย incremental chunks และไม่ append เกิน `max_response_bytes`; Issue #67 กำหนดให้ remote-first session shutdown สำเร็จได้เมื่อส่ง FIN ไม่ได้เพราะ data channel ปิดแล้ว โดย error อื่นยังต้องถูกส่งต่อ; application controls ไม่แทน OS sandbox, ACL หรือ firewall และ `wss` ของ proxy ยังถูกปฏิเสธจนกว่าจะมี tested TLS connector | Threat model และ ADR-047 versioned; browser evidence เป็น local control flow เท่านั้น; production release gate ยังเปิด |

## Known Risks

ความเสี่ยงสูงสุดยังเป็น interoperability กับ client/browser และ signaling server เดิม เพราะ browser surface ใน Issue #47 เป็น local lifecycle/status control เท่านั้น ยังไม่ได้เชื่อม browser WebRTC หรือ remote signaling และยังไม่มี original-client/browser end-to-end capture การ compile ผ่านเพียงอย่างเดียวจึงไม่เพียงพอที่จะยืนยันว่า protocol ใช้งานร่วมกับต้นฉบับได้

ความเสี่ยงรองลงมาคือ cross-platform behavior ของ PTY, filesystem, networking และ Android packaging รวมถึง residual risk ที่บันทึกใน [ADR-046](decisions/issue-46-threat-model.md): signaling preflight ยังมี hostname resolve/connect TOCTOU, dependency advisory scan ยังไม่มีหลักฐานใน sandbox, Windows/Android ต้องอ่านจาก runner จริง และ original-Go/provider interoperability ยังไม่ผ่าน; Issue #67 แก้เฉพาะ error race ของ remote-first close ที่ตรวจพบจาก Windows compatibility test ยังต้องยืนยันบน Windows runner หลัง PR ใหม่; Android เป็น compile-only ไม่มี device/emulator, packaging หรือ runtime evidence; sandbox local compile ยังติด NDK/clang แต่ workflow provisioning ถูกเพิ่มไว้สำหรับ runner; `EndpointPolicy` และ proxy policy ไม่ได้ทดแทน OS/network egress controls
...
| 3 | เพิ่ม typed errors, logging, config และ CLI routing | **ผ่านใน Issues #43 และ #71:** local และ remote fixture/relay commands เรียก business logic จริง พร้อม typed timeout/error cleanup |
...
| 6 | Implement signaling และ WebRTC peer/data channel | **ผ่านใน Issue #71 สำหรับ deterministic relay:** register/request/offer/answer, non-trickle ICE, SWSP data channel และ authenticated session เชื่อมกันจริง; ยังเหลือ external provider/original-Go interoperability และ production NAT traversal |
| 7 | Implement session/auth และ stream registry | **ผ่านใน Issues #11, #38, #39, #40, #42, #43, #44 และ #71:** typed lifecycle, PIN retry/delay, protobuf control handshake, runtime stream cleanup, file/shell/proxy boundaries, local CLI transcripts, deterministic remote relay dispatch และ authenticated compatibility transcript ผ่าน; เหลือ external interoperability tests |
| 8 | เพิ่ม shell/HTTP/TCP/WebSocket, mDNS และ QR | **ผ่านบางส่วนใน Issues #40, #42, #47 และ #71:** shell/file remote dispatcher, concrete proxy handlers และ loopback browser lifecycle/status surface มี local/deterministic coverage; ยังเหลือ TCP/WebSocket dispatcher, browser WebRTC/TLS-enabled interoperability, mDNS/QR และ platform notes |
