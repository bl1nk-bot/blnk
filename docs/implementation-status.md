...
**สถานะเอกสารฉบับนี้:** foundation runnable, identity/pairing primitives, SWSP raw frame codec, typed signaling/session/stream boundaries, deterministic protobuf generation boundary, local WebSocket signaling transport, WebRTC peer lifecycle สำหรับ local two-peer harness, authenticated session runtime บน local data channel, Issue #41 proxy security policy boundary และ Issue #42 concrete TCP/WebSocket/HTTP proxy handlers ถูก implement ใน dependency branches แล้ว; SessionRuntime dispatch, external interoperability fixtures และ production readiness ยังไม่เสร็จ

## Executive Summary

blnk Rust **มี runnable foundation ตามสถาปัตยกรรมแล้ว** และ Issue #7 เพิ่ม identity/pairing primitives, Issue #9 เพิ่ม SWSP raw frame codec, Issue #11 เพิ่ม typed signaling/session/stream boundaries, Issue #13 เพิ่ม deterministic protobuf generation boundary, Issue #33 เพิ่ม JSON-over-WebSocket transport กับ local loopback fixture ที่ทดสอบได้, Issue #35 เพิ่ม public-only signaling endpoint preflight พร้อม explicit local-fixture opt-in, Issue #37 เพิ่ม WebRTC offer/answer, non-trickle ICE, data-channel lifecycle และ deterministic local two-peer harness, Issue #38 เพิ่ม authenticated session runtime, protobuf control exchange, retry/timeout/duplicate protection และ stream cleanup บน local data channel และ Issue #42 เพิ่ม concrete TCP/WebSocket/HTTP proxy service พร้อม local fixtures แต่ยังไม่พร้อมใช้งานจริงหรือ deploy ระบบหลัก เพราะยังเหลือ SessionRuntime dispatch, external-provider interoperability fixtures, web integration และ cross-platform adapters [1] [2]
...
| Stream registry | Issue #11 มี non-zero stream ID allocation, lifecycle validation, counters และ cleanup; Issue #38 ผูก `open_stream`/`close_stream`/disconnect กับ runtime snapshot และพิสูจน์ active resource cleanup ใน local E2E | Registry/runtime lifecycle implemented; concrete handlers exist but dispatch pending |
| Proxy security policy | Issue #41 เพิ่ม `ProxyPolicy` แบบ deny-by-default, exact allowlist/user-confirmation gate, scheme/port/credential/fragment validation, DNS answer-set validation, special-use/mapped-IP rejection, redirect/rebinding guards, bounded resources, stable error mapping และ log/header redaction พร้อม adversarial unit tests | Policy boundary implemented; used by Issue #42 handlers |
| Concrete proxy handlers | Issue #42 เพิ่ม `ProxyStreamService` สำหรับ TCP, `ws` และ one-request HTTP, pinned DNS dialing, manual redirect validation, concurrency/backpressure/size/time limits, SWSP/protobuf open/data codecs และ local echo/HTTP fixtures; `wss` ถูกปฏิเสธจนกว่าจะมี explicit TLS connector | Local concrete handlers implemented; SessionRuntime dispatch, TLS `wss`, platform and interoperability evidence pending |
...
| 8 | เพิ่ม shell/file/HTTP/TCP/WebSocket, mDNS และ QR | Shell/File MVP อยู่ใน PR #53/#54; proxy policy ผ่านใน Issue #41; concrete TCP/WebSocket/HTTP handlers ผ่าน local fixture ใน Issue #42 และต้องเชื่อมกับ SessionRuntime พร้อม external/platform evidence |
...
[13]: ../docs/decisions/issue-42-proxy-streams.md "ADR-042 concrete proxy streams"
[14]: ../src/stream/proxy_handler.rs "Concrete proxy stream service and local fixtures"
