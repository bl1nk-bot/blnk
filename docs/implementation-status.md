...
| Security threat model | Issue #46 เพิ่ม ADR `docs/decisions/issue-46-threat-model.md` ครอบคลุม identity/pairing, signaling, WebRTC/session, file, shell, proxy, dependencies, logging และ platform boundary โดยทุก finding มี severity, evidence, owner และ disposition; `docs/api.md` เพิ่ม capability security boundaries | Local code/tests มี hardening evidence; dependency advisory scan, Windows/Android runner evidence และ original-Go/provider interoperability ยัง pending |
...
| Security boundary | Issue #46 กำหนดว่า authenticated `Ready` state และ capability-specific policy ต้องผ่านก่อนเปิด file/shell/proxy; application controls ไม่แทน OS sandbox, ACL หรือ firewall และ `wss` ยังถูกปฏิเสธจนกว่าจะมี tested TLS connector | Threat model versioned; local MVP boundary documented; production release gate ยังเปิด |
