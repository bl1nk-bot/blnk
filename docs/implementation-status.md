...
| Peer lifecycle | Issue #37 มี `PeerHandle` สำหรับสร้าง peer, offer/answer, non-trickle ICE gathering, connected/failed/closed state, data-channel open/error/close, SWSP send/receive และ idempotent teardown; `TwoPeerHarness` พิสูจน์ local loopback path โดยไม่ใช้ external network; Issue #38 ใช้ `PeerHandle` เป็น control/data boundary ของ authenticated session runtime และ bounded graceful close; Issue #67 ทำให้ `SessionRuntime::close` ยอมรับ remote-first data-channel close แบบจำเพาะโดยไม่กลบ error อื่น | Local peer/session lifecycle implemented; Windows runner ต้องยืนยัน regression scenario อีกครั้ง; signaling and external interoperability pending |
| Session/auth | Issue #11 มี explicit connecting/authenticating/ready/closed state machine, PIN retry/delay policy, typed control messages และ cleanup on terminal failure; Issue #38 เพิ่ม `SessionRuntime` เชื่อม protobuf control messages กับ `PeerHandle`; Issue #39 ใช้ runtime boundary เดิมสำหรับ file request/data/FIN; Issue #43 ใช้ handshake จริงใน CLI local fixture; Issue #67 เพิ่ม safe remote-first shutdown หลัง authenticated handshake | Local runtime/file/CLI integration implemented; Windows close-race runner evidence and original-client interoperability pending |
