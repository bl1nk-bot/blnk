...
| TM-008 | Control/session ปฏิเสธ duplicate หรือ premature `connect`, `auth_required`, `auth_result`, `ready`; timeout และ terminal failure ปิด runtime/peer พร้อม cleanup stream; remote-first data-channel close ระหว่างส่ง `FIN` ถูกจัดเป็น idempotent teardown เฉพาะกรณี error ที่รู้จัก | High | `SessionRuntime::handle_*`, `fail_session`, `connect_seen`, `last_auth_pin`, `is_data_channel_closed_send_error` และ tests สำหรับ duplicate connect, wrong-PIN exhaustion, control timeout, remote-first close และการส่งต่อ error อื่น [9] [16] | Session maintainers | **Mitigated ใน local authenticated runtime**; original-client ordering/replay behavior ยังต้องมี interoperability capture |
...
| WebRTC/session | No unbounded ICE claim, PIN retry exhaustion, fixed-size comparison, duplicate rejection, timeout, remote-first close handling และ cleanup | Peer/session runtime tests [8] [9] [16] | ผ่าน local harness; external evidence pending |
...
| Platforms | Linux full gate, Windows build/test runner, Android compile-only runner, no macOS claim; close ต้องยอมรับ remote-first teardown บนทุก target | Workflow/toolchain, ADR #45 และ Issue #67 [15] [16] | Linux local; runner evidence pending |
...
[16]: ../../src/session/runtime.rs "SessionRuntime remote-first close handling and regression tests"
